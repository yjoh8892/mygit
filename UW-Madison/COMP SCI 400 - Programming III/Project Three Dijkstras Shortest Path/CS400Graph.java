...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public void extend(Edge edge) {
            ...
            this.dataSequence.add(vertex.data);
            this.end = vertex;
        }
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        ...
        PriorityQueue<Vertex> pq = new PriorityQueue<>((Comparator<Vertex>) (o1, o2) -> {
            if (!paths.contains(o1.data))
                return -1;
            if (!paths.contains(o2.data))
                return 1;
            return paths.get(o1.data).compareTo(paths.get(o2.data));
        });


        Vertex startVertex = vertices.get(start);
        paths.put(start, new Path(startVertex));
        pq.add(startVertex);
        ...
        while (!pq.isEmpty()) {
            ...
            Path currPath = paths.get(current.data);
            for (Edge e : current.edgesLeaving) {
                Vertex target = e.target;
                if (explored.contains(target)) {
                    continue;
                }
                int alt = currPath.distance + e.weight;
                Path targetPath = paths.get(target.data);
                if (targetPath == null || alt < targetPath.distance) {
                    Path newPath = new Path(currPath);
                    newPath.extend(e);
                    paths.put(target.data, newPath);
                    pq.remove(target);
                    pq.add(target);
                }
            }
